...
# ═══════════════════════════════════════════════════════════════════
# HOME OVERVIEW DASHBOARD — Single optimised endpoint
# ═══════════════════════════════════════════════════════════════════
...
async def get_dashboard_overview(
    ...
    """
    Comprehensive Overview Dashboard endpoint.
    Returns all KPIs, charts, and lists needed for the Home tab in one call.
    Uses 7-day window for trend data, all-time for counts where sensible.
    """
    tenant = get_tenant_for_user(current_user, db)
    tenant_id = tenant.id if tenant else None
    ...
    conv_7d = Conversation.created_at >= week_ago
    order_7d = CustomerOrder.created_at >= week_ago
    ...
    resolved_conversations = db.query(func.count(Conversation.id)).filter(
        conv_tf, Conversation.status == "resolved"
    ).scalar() or 0
    ...
    positive_count = db.query(func.count(Conversation.id)).filter(
        conv_tf, Conversation.current_sentiment == "positive"
    ).scalar() or 0
    neutral_count = db.query(func.count(Conversation.id)).filter(
        conv_tf, Conversation.current_sentiment == "neutral"
    ).scalar() or 0
    negative_count = db.query(func.count(Conversation.id)).filter(
        conv_tf, Conversation.current_sentiment == "negative"
    ).scalar() or 0
    total_sentiment = positive_count + neutral_count + negative_count
    satisfaction_pct = round((positive_count / max(total_sentiment, 1)) * 100)
    ...
    # Escalations
    escalated_count = db.query(func.count(Conversation.id)).filter(
        conv_tf, Conversation.is_escalated == True
    ).scalar() or 0
    escalation_rate = round(
        (escalated_count / max(total_conversations, 1)) * 100, 1
    )
    ...
    avg_response_time_str = "< 2s"
    try:
        conv_ids = [c.id for c in db.query(Conversation.id).filter(conv_tf).limit(50).all()]
        if conv_ids:
            response_times = []
            for cid in conv_ids:
                msgs = db.query(Message).filter(
            ...
            if response_times:
                avg_rt = sum(response_times) / len(response_times)
                avg_response_time_str = f"{avg_rt:.1f}s" if avg_rt < 60 else f"{avg_rt / 60:.1f}m"
    except Exception:
        pass
    ...
    # ════════════════════════════════════════
    # 3) 7-DAY CONVERSATION VOLUME SPARKLINE
    # ════════════════════════════════════════
    ...
    order_status_rows = db.query(
        CustomerOrder.status,
        func.count(CustomerOrder.id).label("count")
    ).filter(order_tf).group_by(CustomerOrder.status).all()
    ...
    # ════════════════════════════════════════
    # 5) SYSTEM HEALTH (widget, products, KB)
    # ════════════════════════════════════════
    ...
    # ════════════════════════════════════════
    # 6) RECENT ESCALATIONS (last 5)
    # ════════════════════════════════════════

    recent_escalations_q = db.query(Conversation).filter(
        conv_tf,
        Conversation.is_escalated == True
    ).order_by(Conversation.escalated_at.desc()).limit(5).all()
    ...
    recent_escalations = [
        ...
        for esc in recent_escalations_q
    ]

    # ════════════════════════════════════════
    # 7) RECENT CONVERSATIONS (last 5 active)
    # ════════════════════════════════════════

    recent_convs_q = db.query(Conversation).filter(
        conv_tf
    ...
    recent_conversations = [
        {
            ...
            "language_flag": c.language_flag or "🌐",
            "updated_at": c.updated_at.isoformat() if c.updated_at else None,
        }
        for c in recent_convs_q
    ]
